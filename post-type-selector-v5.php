...
class acf_field_post_type_selector extends acf_field {
	...
	function render_field_settings( $field ) {
		...
		*/
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Post type selector', 'acf-select_type'),
			'instructions'	=> __('How would you like to select the post types','acf-select_type'),
			'type'			=> 'select',
			'name'			=> 'select_type',
			'choices' => array(
		...
		foreach ($post_types as $key => $post) {
			$types[$key] = $post->labels->name;
		};

		acf_render_field_setting( $field, array(
			'label'			=> __('Post type to include', 'acf-post_types_to_include'),
			'instructions'	=> __('Which types to include?','acf-post_types_to_include'),
			'type'			=> 'checkbox',
			'name'			=> 'post_types_to_include',
			'choices' => $types
		));
	}
	...
	*/
	...
	function render_field( $field ) {
		...
		$allowed_post_types = $field['post_types_to_include'];
		...
		foreach ( $post_types as $key => $type ) {
			if ( !in_array( $key, $allowed_post_types ) ) {
		...
		switch ( $field[ 'select_type' ] ) {
			...
			case acf_field_post_type_selector::SELECTOR_TYPE_RADIO:
				...
				?>
					...
					<li><label><input type="radio" <?php echo ( isset( $checked[ $post_type->name ] ) ) ? $checked[ $post_type->name] : null; ?> class="<?php echo $field[ 'class' ]; ?>" name="<?php echo $field[ 'name' ]; ?>" value="<?php echo $post_type->name; ?>"><?php echo $post_type->labels->name; ?></label></li>
